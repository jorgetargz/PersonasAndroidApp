package com.jorgetargz.recycler.data.room.common

object Constantes {
    const val DATABASE_NAME = "personas_database"
    const val DATABASE_PATH = "database/personas.db"
    const val NAMED_INJECT_DB = "assetDB"

    const val TABLE_PERSONAS = "personas"
    const val TABLE_HOTELES = "hoteles"
    const val TABLE_PERSONAS_HOTELES = "personas_hoteles"
    const val COLUMN_EMAIL = "email"
    const val COLUMN_CIF ="cif"
    const val COLUMN_NOMBRE_PERSONA = "nombre_persona"
    const val COLUMN_TELEFONO_PERSONA = "telefono_personal"
    const val COLUMN_FNACIMIENTO = "fecha_nacimiento"
    const val COLUMN_NOMBRE_HOTEL = "nombre_hotel"
    const val COLUMN_ESTRELLAS = "estrellas"
    const val COLUMN_TELEFONO_RECEPCION = "telefono_recepcion"
    const val COLUMN_VALORACION = "valoracion"

}